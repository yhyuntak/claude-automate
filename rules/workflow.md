...
> Git branching strategy + tag-based releases + backlog management
...
**Recommended for**: Personal projects, small-scale projects
...
**Recommended for**: Team projects, projects requiring separate deployment environments
...
|------|---------|--------------|---------|
| `feature/*` | New features | `develop` | `feature/user-auth` |
| `fix/*` | Bug fixes | `develop` | `fix/login-error` |
| `hotfix/*` | Emergency fixes | `main` + `develop` | `hotfix/security-patch` |
...
The backlog is managed in the `docs/backlogs/` folder with a todo/doing/done structure.

### Integrated Workflow

**1. Select and start a task**
```bash
# Select a task from todo/ folder in README.md
...
# Update README.md (status, link path, status counts)
```
...
chore:    other (dependencies, configuration, etc.)
style:    code style

...
**확인 질문**:
"brainstorm을 통해 요구사항을 정리하셨나요?"

**판단 기준**:
- **예** → Phase 1로 바로 이동
- **아니오/불확실** → `/brainstorm` 제안

**게이트 통과 조건** (하나라도 만족 시):
- 이전 brainstorm 결과 참조
- 명확한 Context/What/Success 언급
- 사용자가 "요구사항 정리 완료" 확인

**빠른 체크만 수행** - 상세 검증은 brainstorm에서 이미 완료됨
...
2. **게이트 체크**: brainstorm 수행 여부 먼저 확인
3. **devil 1회**: 계획 검증(완성 후)만 수행 (현실성 검증은 brainstorm에서 완료)

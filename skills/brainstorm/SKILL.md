...
### Phase 4.5: devil 😈 현실성 검증

요구사항 정리 후, devil 에이전트로 현실성 검증:
...
**검증 결과에 따른 처리:**
- 경고 없음 → 다음 단계 (/planning) 진행
- 경고 있음 → 사용자에게 경고 공유 후 수정 여부 확인
...
## 다음 단계
→ devil 검증 후 `/planning {feature}` 실행하여 구현 계획 수립
```
...
4. **현실성 검증**: devil로 요구사항 검증 후 planning 진행
5. **다음 단계 안내**: planning으로 연결
